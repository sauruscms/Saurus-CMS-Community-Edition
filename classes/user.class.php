...
function get_sso() {
	...
	if($this->user_id){
		...
		# loadime k�ik selle kasutaja rakenduste login info massiivi
		$sql = $site->db->prepare(
...
#		echo 'whatever role by ID: '.$role_id;
	...
	#		echo printr($permrole[$role_id]);
		...
		# NB! Siin on erand grupi "Everybody" korral (mille abil keelatakse KEELA K�IGILE reegel): 
		# juhul kui kasutaja kuulub ka m�nda rolli siis ignoreeritakse Everybody grupi KEELAVAID �iguseid. 
		...
		foreach($grouptree as $key=>$group){
			...
			if(is_array($permgrp[$group['id']])){
				foreach( array_keys($permgrp[$group['id']]) as $key2=>$obj_id) {
					...
					# kui grupp on Everybody (ID=1) JA leidub rolli-�igused selle objekti kohta,					
					if($group['id'] == 1 && sizeof($cemented_perm[$obj_id]) > 0) {
						# siis ignoreeri Everybody grupi K�IKI �iguseid selle objekti kohta, sest peavad m�juma rolli omad. (Bug#2552, #2701)
						continue;
					}
