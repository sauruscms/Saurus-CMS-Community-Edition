...
(function($)
{
	$.fn.contextMenu = function(buttons)
	{
		...
		return this.each(function(i)
		{
			...
			{
				if ($.inArray(argButtons[i], buttons) == -1)
					buttons.push(argButtons[i]);
			}
	...
	$.fn.contextMenu.openMenu = function(data)
	{
		...
		if (contextMenu.length == 0)
		{
			...
			$('body:first').prepend('<ul id="context_plugin_dropdown_' + data.index + '" class="context_plugin_dropdown dropdown-menu"></ul>');
			...
			for (var i in data.buttons)
				if ($.fn.contextMenu.actions[data.buttons[i]])
				{
					actionItems += '<li class="context_plugin_item">'+
									'<a class="' + $.fn.contextMenu.actions[data.buttons[i]].name + '" href="javascript:void(0);">'+$.fn.contextMenu.actions[data.buttons[i]].title+'</a>'+
									'</li>';
				}
	...
	$.fn.contextMenu.addAction = function(action)
	{
		...
		// bind menu hiding
		$(document).on('click', 'a.' + action.name, function()
		{
			$('ul.context_plugin_dropdown').hide();
		});

		// bind all future clicks
		$(document).on('click', 'a.' + action.name, action.bind);
	};
	...
	$.fn.getAttributes = function(filter)
	{
		...
		this.each(function()
		{
			...
			for (var i in this.attributes)
				if (this.attributes[i] && this.attributes[i].nodeName != undefined)
				{
					if (filter == undefined)
					{
